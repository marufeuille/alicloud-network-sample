...
resource "alicloud_ssl_vpn_server" "ssl-vpn-server" {
  ...
  local_subnet   = "${alicloud_vswitch.vsw_region-a.cidr_block}"
  protocol       = "UDP"
  cipher         = "AES-128-CBC"
  port           = 1194
  compress       = "false"
...
resource "alicloud_instance" "proxy-a" {
  ...
  user_data            = "#!/bin/bash\necho \"${file("static/squid/squid.conf")}\" > /tmp/squid.conf\n${data.template_file.prv-proxy.rendered}"
}
...
data "template_file" "prv-proxy" {
  template = "${file("templates/provisioning-proxy.tpl")}"
  vars     = {
